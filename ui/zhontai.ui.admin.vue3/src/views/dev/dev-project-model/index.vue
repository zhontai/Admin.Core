  ...
  <MyLayout>
    ...
    <el-card class="my-fill mt8" shadow="never">
      ...
      <el-table v-loading="state.loading" :data="state.devProjectModelListData" row-key="id" ref="listTableRef" border @selection-change="selsChange">
        ...
        <el-table-column prop="name" :label="t('模型名称')" show-overflow-tooltip width />
        <el-table-column prop="code" :label="t('模型编码')" show-overflow-tooltip width />
        ...
        <el-table-column prop="remark" :label="t('备注')" show-overflow-tooltip width />
        <el-table-column
...
const onAdd = () => {
  state.devProjectModelFormTitle = t('新增项目模型')
  devProjectModelFormRef.value.open()
}

const onEdit = (row: DevProjectModelGetOutput) => {
  state.devProjectModelFormTitle = t('编辑项目模型')
