        ...
        <el-row :gutter="35">
          <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
            <el-form-item
              :label="t('任务标题')"
              prop="topic"
              :rules="[{ required: true, message: t('请输入任务标题'), trigger: ['blur', 'change'] }]"
            >
              <el-input v-model="form.topic" clearable />
            </el-form-item>
          </el-col>
          <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
            <el-form-item :label="t('任务参数')" prop="body">
              <template #label>
                <div class="my-flex-y-center">
                  {{ t('任务参数') }}
                  <el-tooltip effect="dark" placement="top" hide-after="0">
                    <template #content>{{ t('设置Json数据') }}</template>
            ...
            <el-form-item prop="alarmEmail">
              <template #label>
                <div class="my-flex-y-center">
                  {{ t('报警邮件') }}
                  <el-tooltip effect="dark" placement="top" hide-after="0">
                    <template #content>{{ t('多个邮件地址用逗号分隔') }}</template>
                    <SvgIcon name="ele-InfoFilled" class="ml5" />
                  </el-tooltip>
                </div>
              </template>
              <el-input v-model="form.alarmEmail" clearable :placeholder="t('多个邮件地址用逗号分隔')" />
            </el-form-item>
          </el-col>
          <el-col :xs="24" :sm="24" :md="12" :lg="12" :xl="12">
            <el-form-item prop="failRetryCount">
              <template #label>
                <div class="my-flex-y-center">{{ t('失败重试次数') }}</div>
              </template>
              <el-input-number class="w100" v-model="form.failRetryCount" :min="0" />
            </el-form-item>
          </el-col>
          <el-col :xs="24" :sm="24" :md="12" :lg="12" :xl="12">
            <el-form-item prop="failRetryCount">
              <template #label>
                <div class="my-flex-y-center">{{ t('重试间隔（秒）') }}</div>
              </template>
              <el-input-number class="w100" v-model="form.failRetryInterval" :min="0" />
            </el-form-item>
          </el-col>
          <el-col :xs="24" :sm="24" :md="12" :lg="12" :xl="12">
            <el-form-item prop="round" :rules="[{ required: true, message: t('请输入执行轮数'), trigger: ['blur', 'change'] }]">
              <template #label>
                <div class="my-flex-y-center">
                  {{ t('执行轮次')
                  }}<el-tooltip effect="dark" placement="top" hide-after="0">
                    <template #content>{{ t('循环多少次，-1为无限循环') }}</template>
                    <SvgIcon name="ele-InfoFilled" class="ml5" />
                  </el-tooltip>
                </div>
              </template>
              <el-input-number class="w100" v-model="form.round" :min="-1" :disabled="form.interval === 21" />
            </el-form-item>
          </el-col>
          <el-col :xs="24" :sm="24" :md="12" :lg="12" :xl="12">
            <el-form-item :label="t('定时类型')" prop="interval" :rules="[{ required: true, message: t('请选择定时类型'), trigger: ['change'] }]">
              <el-select v-model="form.interval" :options="intervals" @change="onIntervalChange"> </el-select>
            </el-form-item>
          </el-col>
          <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
            <el-space fill class="w100">
              <el-form-item
                ...
                :rules="[{ required: true, message: t('请输入定时参数'), trigger: ['blur', 'change'] }]"
              >
              ...
              <el-alert v-if="form.interval === 1" type="info" :closable="false">
                {{ t('设置 5 则每5秒触发，执行N次') }}
                <br />
                {{ t('设置 5, 5, 10, 10, 60, 60 则每次按不同的间隔秒数触发，执行6次') }}
              </el-alert>
              <el-alert v-else-if="form.interval === 11" type="info" :closable="false">
                {{ t('设置 08:00:00 则每天 08:00:00 触发，执行N次') }}
              </el-alert>
              <el-alert v-else-if="form.interval === 12" type="info" :closable="false">
                {{ t('设置 1:08:00:00 则每周一 08:00:00 触发') }}
                <br />
                {{ t('设置 0:08:00:00 则每周日 08:00:00 触发') }}
              </el-alert>
              <el-alert v-else-if="form.interval === 13" type="info" :closable="false">
                {{ t('设置 1:08:00:00 则每月1日 08:00:00 触发') }}
                <br />
                {{ t('设置 -1:08:00:00 则每月最后一日 08:00:00 触发') }}
              </el-alert>
              <el-alert v-else-if="form.interval === 21" type="info" :closable="false">
                {{ t('设置 0/10 * * * * ? 则从0秒开始每10秒执行一次') }}
                <br />
...
{
    //{{ t('利用 cron 功能库解析 task.IntervalArgument 得到下一次执行时间') }}
    //{{ t('与当前时间相减，得到 TimeSpan，若返回 null 则任务完成') }}
...
const intervals = computed(() => [
  { label: t('按秒触发'), value: 1 },
  { label: t('每天'), value: 11 },
  { label: t('每周几'), value: 12 },
  { label: t('每月第几日'), value: 13 },
  { label: t('Cron表达式'), value: 21 },
])
